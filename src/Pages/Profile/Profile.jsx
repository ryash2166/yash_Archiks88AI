...
const ImageSkeleton = () => {
  return (
    <div className="animate-pulse bg-gray-700 rounded-[18px] mb-3">
...
// Add this where you define your other state variables
// const [loadingFailed, setLoadingFailed] = useState(false);
// Enhance the loadMoreImages function with error handling
...
// };
  ...
  // Update your last image ref callback
...
//       if (entries[0].isIntersecting && hasMoreImages && !loadingFailed) {
//         handleLoadMore();
//       }
//     });
...
//   },
//   [isImagesLoading, hasMoreImages, loadingFailed]
// );
  ...
  if (isProfileLoading && firstLoad) {
    return (
      <div className="flex items-center justify-center min-h-screen bg-[#0d1116]">
        <div className="flex flex-col items-center gap-4">
          <div className="animate-spin rounded-full h-12 w-12 border-t-2 border-b-2 border-[#72e528]"></div>
  ...
  return (
    <div className="relative h-lvh w-full overflow-auto">
      <div className="lg:pl-[260px] pt-0 m-0 bg-[#0d1116]">
        ...
        <div className="max-md:mt-24 mb-24 mx-8">
          {!profile?.images?.length && <ProfileTabs />}
          <ResponsiveMasonryWrapper
            className="mt-3"
            columnsCountBreakPoints={{ 576: 1, 767: 2, 1024: 4 }}
          >
            {profile?.images && profile.images.length > 0 && (
              <div className="sticky top-0 bg-[#0d1116] z-10 text-white p-6 w-full shadow-xl text-3xl max-md:text-xl font-semibold flex justify-between items-center">
                <span>Your Creativity</span>
                <button 
                  className="text-sm bg-[#1c262b] hover:bg-[#2a383f] px-4 py-2 rounded-full"
            ...
            <Masonry className="!m-auto" gutter="10px">
              {profile.images && profile.images.length > 0 ? (
                profile.images.map((img, index) => {
                  // Apply ref to last image for infinite scroll
                  const isLastImage = index === profile.images.length - 1;
                  return (
                    <div 
                      key={img._id || index} 
                      ref={isLastImage ? lastImageRef : null}
                      className="relative inline-block group mb-3"
                    >
                      <img
                        src={img.imageUrl}
                        alt={img.prompt}
                        className="cursor-pointer rounded-[18px] w-full"
                        loading="lazy"
                        
                      />
                      <div className="absolute inset-0 flex items-center justify-center bg-black bg-opacity-0 group-hover:bg-opacity-50 transition-all duration-300 cursor-pointer rounded-[18px]" onClick={() =>
                            handleDownload(img.imageUrl, `${img._id}.png`)
                          }>
                        <FiDownload
                          className="text-white text-4xl opacity-0 group-hover:opacity-100 transition-opacity duration-300"
                ...
                <p className="text-white text-center w-full">No images generated yet.</p>
              )}
              
              {/* Loading skeletons */}
              {isImagesLoading && !firstLoad && hasMoreImages && (
                <>
                  <ImageSkeleton />
                  <ImageSkeleton />
                  <ImageSkeleton />
...
{/* {hasMoreImages && (
  <div className="col-span-full flex justify-center my-4">
    {isImagesLoading && !firstLoad ? (
      <div className="animate-spin rounded-full h-8 w-8 border-t-2 border-b-2 border-[#72e528]"></div>
    ) : loadingFailed ? (
      <button 
        onClick={handleLoadMore} 
        className="bg-[#1c262b] hover:bg-[#2a383f] text-white px-4 py-2 rounded-full"
      >
        Try Again
      </button>
    ) : (
      ...
      {isModalOpen && (
        <div
          ...
          onClick={handleModalClick}
        >
          <div className="bg-[#15181d] rounded-lg p-6 w-full max-w-md mx-2">
            ...
            <form onSubmit={saveProfile} className="space-y-4">
              <div className="flex justify-center mb-4">
                <div className="relative w-24 h-24">
                  ...
                  <label
                    htmlFor="avatar-upload"
                    className="absolute bottom-0 right-0 bg-[#72e528] rounded-full p-1 cursor-pointer"
                  >
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      fill="none"
              ...
              <div>
                ...
                <input
                  ...
                  value={tempProfile.name || ""}
                  onChange={handleInputChange}
                  className="w-full p-2 bg-[#1c262b] border border-gray-700 rounded-md text-white focus:outline-none focus:ring-2 focus:ring-[#72e528] focus:border-transparent"
                  maxLength={30}
                />
              </div>
              <div>
                <div className="flex justify-between items-center mb-1">
                  ...
                  <button
                    type="button"
                    onClick={clearBio}
                    className="text-xs text-[#72e528] hover:underline"
                  >
                    Clear
                  </button>
                </div>
              ...
              <div className="flex justify-end space-x-3">
                <Button
                  onClick={closeModal}
                  variant="outline"
                  className="px-4 py-2 border border-gray-600 text-white rounded-full hover:bg-gray-700 transition-colors"
                >
                  Cancel
                </Button>
                <Button
                  type="submit"
                  className="px-4 py-2 bg-[#72e528] text-black rounded-full hover:bg-[#5fc31b] transition-colors"
                >
                  Save Changes
                </Button>
              </div>
